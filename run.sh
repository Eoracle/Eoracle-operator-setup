...
if [ "$1" == "encrypt" ]; then
    ...
    docker run --rm --env-file .env --env-file .private_key --platform linux/amd64 ${private_path} ${EO_OPERATOR_CLI_IMAGE} ${1}
    rm .private_key > /dev/null 2>&1
elif [ "$1" == "register" ] || [ "$1" == "deregister" ] || [ "$1" == "print-status" ] || [ "$1" == "decrypt" ] || [ "$1" == "generate-bls-key" ] || [ "$1" == "eochain-set-alias" ]; then
    _cmd=$1
    shift
    docker run --rm --env-file .env ${private_path} --platform linux/amd64 ${EO_OPERATOR_CLI_IMAGE} "${_cmd} $@"
else    
    # invalid arg
